			...
			<div class="col-md-12">
				<?php 
					if(in_array("admin_authorization", explode(",", $active_query_user["pages"]))){
						if($_GET["id"]!=1){
							$id=$_GET["id"];
							echo message("info", "info", "Bilgi", "Lütfen yeni yetki eklerken yetki sıralamasının üst rütbeden alt rütbeye dogru sıralı olmasına dikkat ediniz");
							...
							if($_POST){
								...
								if($password == $active_query_user["password"]){
									...
									if($query){
										...
										echo message("success","check","Başarılı","$name isimli yetki güncellendi");
										_header("admin_authorization");
									}else{
										echo message("warning","exclamation-triangle","Dikkat!","Yetki güncellenemedi");
									}
								}else{
									$pass_result=0;
					...
					<?php
							...
							echo message("danger","ban","Dikkat!","Bu yetki düzenlenemez. Lütfen bekleyiniz yetki işlemleri sayfasına yönlendirliyorsunuz");
							_header("admin_authorization");
						}
					}else{
