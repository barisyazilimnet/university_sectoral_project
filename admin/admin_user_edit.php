			...
			<div class="col-md-12">
				<?php
					if(in_array("admin_user_transactions", explode(",", $active_query_user["pages"]))){
						$id=$_GET["id"];
						if($id!=1){
							$admin_users_query=mysqli_fetch_array(mysqli_query($con, "SELECT admin_users.*, user_authorization.name, user_authorization.authorization_color FROM admin_users LEFT JOIN user_authorization ON admin_users.authorization_id=user_authorization.authorization_id WHERE admin_users.user_id='$id'"));
							...
							if($_POST){
								...
								if($password == $active_query_user["password"]){
									if($turkish==False){
										$photo=upload_edit($_FILES["photo"]["tmp_name"],$_FILES["photo"]["name"],$user_name."_photo_","profile_photos/admins/",
										...
										if($photo){
											...
											if($query){
												...
												echo message("success","check","Başarılı","Üye başarılı bir şekilde güncellendi");
					...
					<div class="col-md-2">
						<div class="card card-info card-outline">
							<div class="card-body box-profile">
								...
								<p class="text-muted text-center"><?php echo _date($admin_users_query["birthday"]); ?></p>
								...
								<p class="text-center"><?php echo _date($admin_users_query["registration_date"],true); ?></p>
							</div>
							<!-- /.card-body -->
						</div>
						<!-- /.card -->
				...
				<?php
						...
						echo message("danger","ban","Dikkat!","Bu kullanıcı düzenlenemez. Lütfen bekleyiniz üye işlemleri sayfasına yönlendirliyorsunuz");
						_header("admin_user_transactions");
					}
				}else{
